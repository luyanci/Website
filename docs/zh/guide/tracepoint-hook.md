...
起，SukiSU 引入了 Tracepoint Hook

**该 Hook 理论上相比于 Kprobes Hook，性能开销更小，但次于 Manual Hook / Syscall
Hook**

::: warning 目前 Tracepoint
Hook 在 6.x 设备上并不稳定，请勿使用，否则可能出现无法开机或无法获取 `ROOT`
权限等问题。:::

> [!NOTE] 本教程参考了
> [backslashxx/KernelSU#5](https://github.com/backslashxx/KernelSU/issues/5)
> 的 syscall hook v1.4 版本钩子，以及原版 KernelSU 的
> [Manual Hook](https://kernelsu.org/guide/how-to-integrate-for-non-gki.html#manually-modify-the-kernel-source)
...
要使用 KernelSU 内置的安全模式，你还需要修改 `drivers/input/input.c`
中的 input_handle_event 方法：
