...
async function handleRequest(request: Request, url: URL): Promise<Response> {
  const isStatic: boolean = CACHE_STRATEGIES.static.some((ext: string) => url.pathname.endsWith(ext));
  const isImage: boolean = CACHE_STRATEGIES.images.some((ext: string) => url.pathname.endsWith(ext));
  const isDocument: boolean = CACHE_STRATEGIES.documents.some((ext: string) => url.pathname.endsWith(ext)) || url.pathname.endsWith('/');
...
async function handleDocument(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (request.mode === 'navigate') {
      const offlinePage: Response | undefined = await cache.match('/');
